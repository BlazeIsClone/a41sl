...
class UserData {
    constructor(region, username) {
        this.base_url = `https://${UserData.Region(
            region
    ...
    async profileBasicData(message) {
        try {
            ...
            if (err.response.status == 404) {
                throw new Error(
                    "This user does not exist in the current region"
                );
            } else {
                throw new Error(
                    "An error has occurred while trying to fetch user data"
                );
            }
        }
    }
        ...
        try {
            ...
            return Promise.all(
                mostPlayedChampions.map(async champion => {
                    ...
                    const { image, name } = await this.getChampionById(
                        championId
                    );
